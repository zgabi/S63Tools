...
public sealed class BlowFish : IDisposable
{
    ...
    private static readonly uint[] SetupP = {
        0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0,
        0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c,
        0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917, 0x9216d5d9, 0x8979fb1b,
    };

    private static readonly uint[] SetupS0 = {
    ...
    private static readonly uint[] SetupS1 = {
    ...
    private static readonly uint[] SetupS2 = {
    ...
    private static readonly uint[] SetupS3 = {
    ...
    private RandomNumberGenerator _randomSource = RandomNumberGenerator.Create();

    // SBLOCKS
    private readonly uint[] _bfS0 = new uint[256];
    private readonly uint[] _bfS1 = new uint[256];
    private readonly uint[] _bfS2 = new uint[256];
    private readonly uint[] _bfS3 = new uint[256];
    private readonly uint[] _bfP = new uint[18];

    // HALF-BLOCKS
    private uint _xlPar;
    private uint _xrPar;
    private readonly byte[] _initVector = new byte[8];
    ...
    public BlowFish(ReadOnlySpan<byte> cipherKey)
    {
        if (cipherKey == null)
        {
    ...
    public IEnumerable<byte> IV
    {
        ...
        set
        {
            if (value == null)
            {
                _initVector.AsSpan().Clear();
                return;
            }

            var iv = value.ToArray().AsSpan();
            if (iv.Length == 8)
            {
                iv.CopyTo(_initVector);
            }
            else
            {
                throw new InvalidOperationException("Invalid IV Size");
            }
        }
    }
    ...
    public byte[] EncryptCBC(Span<byte> pt)
    {
        int paddedLen = pt.Length;
        var pt8 = pt.ToArray();

        if (paddedLen % 8 != 0)
        {
            paddedLen = ((paddedLen >> 3) + 1) << 3;
            pt8 = new byte[paddedLen];
            pt.CopyTo(pt8);
        }
    ...
    /// IV must be created and saved manually.
    /// </summary>
    /// <param name="text">Plaintext data to encrypt.</param>
    public void EncryptCBC8(Span<byte> text)
    {
        XorBlock_private(text, _initVector);
        BlockEncrypt(text);
    }
    ...
    public byte[] DecryptCBC(Span<byte> ct)
    {
        int paddedLen = ct.Length;
        var ct8 = ct.ToArray();

        if (paddedLen % 8 != 0)
        {
            paddedLen = ((paddedLen >> 3) + 1) << 3;
            ct8 = new byte[paddedLen];
            ct.CopyTo(ct8);
        }
    ...
    /// IV must be created and saved manually.
    /// </summary>
    /// <param name="text">Ciphertext data to decrypt.</param>
    public void DecryptCBC8(Span<byte> text)
    {
        BlockDecrypt(text);
        XorBlock_private(text, _initVector);
    }
    ...
    public byte[] EncryptECB(Span<byte> pt)
    {
        if (pt == null) throw new ArgumentNullException(nameof(pt));

        return Crypt_ECB(pt.ToArray(), false);
    }
    ...
    public byte[] Decrypt(byte[] cipherText, CipherMode mode)
        => mode switch
        {
            CipherMode.ECB => Decrypt_ECB(cipherText),
    ...
    public void SetupKey(ReadOnlySpan<byte> cipherKey)
    {
    ...
    private void Crypt_CBC(Span<byte> text, bool decrypt)
    {
        byte[] preblock = null;

        var iv = _initVector.AsSpan();

        for (var i = 0; i < text.Length; i += 8)
        {
            var block = text.Slice(i, 8);
            if (decrypt)
            {
    ...
    private void SetBlock(Span<byte> block)
    {
        ...
        if (!NonStandard)
        {
            _xlPar = BinaryPrimitives.ReadUInt32BigEndian(block);
            _xrPar = BinaryPrimitives.ReadUInt32BigEndian(block.Slice(4));
        }
        else
        {
            _xrPar = BinaryPrimitives.ReadUInt32LittleEndian(block);
            _xlPar = BinaryPrimitives.ReadUInt32LittleEndian(block.Slice(4));
        }
    }
    ...
    private void GetBlock(Span<byte> block)
    {
        if (!NonStandard)
        {
            BinaryPrimitives.TryWriteUInt32BigEndian(block, _xlPar);
            BinaryPrimitives.TryWriteUInt32BigEndian(block.Slice(4), _xrPar);
        }
        else
        {
            BinaryPrimitives.TryWriteUInt32LittleEndian(block, _xrPar);
            BinaryPrimitives.TryWriteUInt32LittleEndian(block.Slice(4), _xlPar);
        }
    }
